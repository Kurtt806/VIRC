...
void initWebSocket()
{
  ws.onEvent([](AsyncWebSocket *server, AsyncWebSocketClient *client,
    ...
    if (type == WS_EVT_DATA) {
      ...
      else if (msg.startsWith("TOGGLE_ID:")) {
        int sep1 = msg.indexOf(':', 10);
        int sep2 = msg.indexOf(':', sep1 + 1);
        if (sep1 > 0 && sep2 > sep1) {
          String id = msg.substring(10, sep1);
          bool enable = msg.substring(sep2 + 1) == "1";
          
          if (vircCfg.count(id)) {
            const auto &map = vircCfg[id];
            String effectName = map.at("effect");
            ...
            for (auto &cfg : led->_effects) {
              if (cfg.name == effectName &&
                ...
                cfg.enabled = enable;
                sendLogToClients("🔁 Toggled (ID): " + id);
                break;
              }
            }
          }
